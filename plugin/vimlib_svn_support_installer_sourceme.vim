...
" some subversion support because the existing solutions didn't work for me
...
" start region 'file <dotvim>/autoload/vl/ui/resultBuffer.vim'
" "|fld   description : helper functions to show arbitrary results within a non
" "|+     file buffer
" 
" " action may be a string eg 'show,add'
" " allowed actions by now: "show" and "add", "focus"
" function! vl#ui#resultBuffer#ResultBufferAction(actions, name, ...)
"   exec vl#lib#brief#args#GetOptionalArg("opt",string(0))
"   let actions = split(a:actions,",")
"   let options = vl#lib#listdict#list#ToList(opt)
" 
"   let is_visible = bufwinnr(bufnr(a:name)) >= 0
"   " this is ignored for now...
"   let focused = winnr() == bufwinnr(bufnr(expand('%')))
" 
"   let opt_idx=0
...
"       if !is_visible
"         exec 'sp '.a:name
"         call vl#ui#resultBuffer#SetBufferOpts()
"       else
"         call vl#ui#resultBuffer#ResultBufferAction("focus", a:name)
"       endif
"     elseif action == 'add'
"       call vl#ui#resultBuffer#ResultBufferAction('show', a:name)
"       normal G
"       silent! put=options[opt_idx]
"       let opt_idx +=1
" elseif action == 'focus'
"       " set focus
"       exec bufwinnr(bufnr(a:name)).' wincmd w'
"     endif
"   endfor
" endfunction
" 
" 
" function! vl#ui#resultBuffer#SetBufferOpts()
"   if exists('b:bufferOptsSet')
"     return 
"   endif
"   let b:bufferOptsSet = 1
"   setlocal ft=resultBuffer
"   setlocal buftype=nofile
"   setlocal noswapfile
"   setlocal noreadonly
" endfunction
" end region 'file <dotvim>/autoload/vl/ui/resultBuffer.vim'
...
" "|fld   version: 0.0
" 
" function! vl#lib#system#system#System( expectedExitCode, command )
"   call vl#lib#ide#logging#Log('executing system command '.a:command)
"   let result = system(a:command)
"   let g:systemResult = result
"   if v:shell_error != a:expectedExitCode
"     let g:systemResult = result
"     throw "command ".a:command."failed with exit code ".v:shell_error
"      \ . "but ".a:expectedExitCode." expected, the first 500 chars of the result are".strpart(result,0,500)
"      \ . "you can get all the text from g:systemResult"
"   endif
"   return result
" endfunction
" 
" " give the args and let the function escape them (TODO fix escaping)
" function! vl#lib#system#system#SystemWA(expectedExitCode, prog, args)
"   let args = join( vl#lib#listdict#list#MapCopy(a:args
"         \ ,"'\"'.substitute(Val,'\"','\\\"','g').'\"'"), " ")
"   return vl#lib#system#system#System(a:expectedExitCode, a:prog." ".args)
" endfunctio"n
" "
" end region 'file <dotvim>/autoload/vl/lib/system/system.vim'


" start region 'file <dotvim>/autoload/vl/ui/userSettings.vim'
...
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Mar 08 02:21:00
" "|fld   version: 0.0
...
" "|H1__  Documentation
" "|
" "|p     let user edit a set of settings in an extra buffer
" "|
" "|TODO  add completion support etc
" 
" "|h2 the dictionary describing one property:
" 
" "| setting dictionary description:
" "|pre
" "|     'catption'   : < the caption to show/ identifier > (mandatory)
" "|     'default'    : <the default value>                 (optinal)
" "|     'mandatory'  : 1                                   (optional)
" "|     'comment'    : a one line text describing the option
" "|     'validation' :  an expression can be given here to evaluate if the value is valid
" "|                     eg "val == 'yes' || val == 'no'"   (mandatory)
" "|                     should return either 0, 1 (failure, success) or { error : "message" }
" "| what else might be needed?
" "|/pre
" "|
" "|h2    example usage:
" "|code  call vl#ui#userSettings#LetUserEditProperties([{'caption': "Name",'validation':'val==2'},{'caption':"Nachname"}], 'vl#ui#userSettings#ContinuationShowProperties')
" 
" "func lets the user edit a list of properties using an extra buffer
" "
" "     the buffer will look like
" "     # comment may only occur above a option line
" "code @#@ <caption1> : <value1>
" "     @#@ <catpion1> : <value2>
" "     @#@ <caption2> : value spanning over two\
" "                    lines
" 
" function! vl#ui#userSettings#ReadPropertiesFromLines(lines)
"   let result = {}
"   let optionR = '@#@ \([^:]*\) :\(.*\)'
"   let fstOpt = 1
"   for l in a:lines
...
" endfunction
" 
" function! vl#ui#userSettings#CheckProperties(properties, readProps)
"   let errors = []
"   let readProps = a:readProps
"   " check mandatory
"   for p in a:properties
"     if vl#lib#listdict#dict#KeyValue(p,'mandatory',0) == 1
"       if !vl#lib#listdict#dict#HasKey(readProps, p['caption'])
" 	call add(errors, "property ". p['caption']. ' is mandatory but property not found')
"       endif
"     endif
"   endfor
"   " trim items
"   for p in a:properties
"     if vl#lib#listdict#dict#KeyValue(p,'trim',0) == 1
"       if vl#lib#listdict#dict#HasKey(readProps, p['caption'])
"         let readProps[p['caption']] = matchstr( readProps[p['caption']], '^\s*\zs\S\{-}\ze\s*$')
"       endif
"     endif
"   endfor
"   " validate items when given
"   let d = {}
"   for i in a:properties
"     let d[i['caption']] = i
"   endfor
"   for key in keys(d)
"     if vl#lib#listdict#dict#HasKey(a:properties, key)
"       let prop = d[key]
"       let val = readProps[key]
"       let validation = vl#lib#listdict#dict#KeyValue(prop, 'validation', string(1))
"       exec 'let valid = '.validation
"       if valid == 1
" 	continue
"       elseif (type(valid) == 4 && vl#lib#listdict#dict#HasKey(valid, 'error'))
" 	let message = ' error :' valid['error']
"       else
" 	let message=''
"       endif
"       call add(errors, "property _".key."_ is not valid".message."\t\t (it should suffice ".validation.")")
"     endif
"   endfor
"   return errors
" endfunction
" 
" function! vl#ui#userSettings#PropertiesWritten(close)
"   let readProps = vl#ui#userSettings#ReadPropertiesFromLines(getline(0,'$'))
"   let errors = vl#ui#userSettings#CheckProperties(b:properties, readProps)
"   if errors == []
"     let cont = b:continuationFunc
"     if a:close && &buftype == 'acwrite'
"       bd!
"     endif
"     call vl#lib#vimscript#closure#Call(cont,[readProps])
"   else
...
" endfunction
" 
" function! vl#ui#userSettings#EditNextProperty()
"   let [a,b] = vl#lib#buffer#splitlineatcursor#SplitCurrentLineAtCursor()
"   call search('^[^:]*:','e')
"   if a =~ ':$' && b == ''
"     startinsert!
"   else
"     startinsert! " I'd like to select it in visual mode. But this is more complicated..
"   endif
" endfunction
" 
" " continuationFunc must be a String. (if you want to use function the function
" " must already exist, which means we can't use autoloading here)
" function! vl#ui#userSettings#LetUserEditProperties(properties, continuationFunc,...)
"   exec vl#lib#brief#args#GetOptionalArg('title',string(''))
"   sp | enew
"   " file dummy
"   "au BufWriteCmd <buffer>  * :call vl#ui#userSettings#PropertiesWritten()<cr>
"   let b:continuationFunc = a:continuationFunc
"   let b:properties = a:properties
"   set buftype=acwrite
"   put=title
"   put=''
"   put='  press <m-h> for a short help, <m-f> when you have finished'
"   put=''
"   for i in range(0, len(a:properties) -1)
"     let p = a:properties[i]
"     put='@#@ '.vl#lib#listdict#dict#KeyValue(p,'caption','no caption given').' :'.vl#lib#listdict#dict#KeyValue(p,'default','')
"   endfor
"   noremap <buffer> <m-h> <esc>:call vl#ui#userSettings#ShowPropertyBufferHelp()<cr>
"   nnoremap <buffer> <m-cr> :call vl#ui#userSettings#EditNextProperty()<cr>
"   inoremap <buffer> <m-cr> <esc>:call vl#ui#userSettings#EditNextProperty()<cr>
"   noremap <buffer> <m-f> :call vl#ui#userSettings#PropertiesWritten(1)<cr>
"   command -buffer Finished :call vl#ui#userSettings#PropertiesWritten(1)<cr>
"   inoremap <buffer> <m-f> <esc>:call vl#ui#userSettings#PropertiesWritten(1)<cr>
"   noremap <buffer> <m-e> :call vl#ui#userSettings#PropertiesWritten(0)<cr>
"   inoremap <buffer> <m-e> <esc>:call vl#ui#userSettings#PropertiesWritten(0)<cr>
"   "noremap <buffer> \f :call vl#ui#userSettings#PropertiesWritten()<cr>
"   "noremap <buffer> <m-c>: /^<c-r>=v:count<cr>)\s*:/e<cr>
"   "noremap <buffer> <m-g>: /^<c-r>=v:count<cr>)\s*:/e<cr>
"   normal gg
"   call search('^[^:]*:','e')
" endfunction
" 
" function! vl#ui#userSettings#ShowPropertyBufferHelp()
"   echo '<m-h>: show this help'
"   echo 'single line properties may be added right after the '':'', exmaple:'
"   echo '@#@ lable one line :value'
"   echo '|'
"   echo 'multiline properties are to be entered below the label, example:'
"   echo '@#@ one or multiline property :'
"   echo 'line 1'
"   echo 'line 2'
"   echo ''
"   "echo 'use 2<m-c> to edit snd item'
"   "echo 'use 2<m-g> to jump to snd item'
"   echo '|'
"   echo '<m-f> to finish'
"   echo '<m-e> to execute continuation function without closing window'
"   echo '<m-cr> to edit next property'
"   echo ':q to cancel'
" endfunction
" 
" function! vl#ui#userSettings#ContinuationShowProperties(props)
"   for k in keys(a:props)
"     echo "key : ". k . ' value : ' . a:props[k]
"   endfor
" endfunction
" end region 'file <dotvim>/autoload/vl/ui/userSettings.vim'


" start region 'file <dotvim>/autoload/vl/dev/versioncontrolsystem/subversion.vim'
" "|fld   description : <+ really short description to get a picture. less than 2 sentences if possible +>
" "|fld   keywords : <+ this is used to index / group scripts ?+> 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Aug 24 01:34:46
" "|fld   version: 0.0
" "|fld   dependencies: <+ vim-python, vim-perl, unix-tool sed, ... +>
" "|fld   contributors : <+credits to+>
" "|fld   tested on os : <+credits to+>
...
" "|      "description
" "let s:<+setting name+>=vl#lib#vimscript#scriptsettings#Load('vl.dev.versioncontrolsystem.subversion.<+setting_name+>',<+ default +>)
" "|
" "|
...
" 
" "function! vl#dev#versioncontrolsystem#subversion#Operations()
"   "let operations = {}
"   "for op in [ 'VCAdd' , 'VCCommit' , 'VCDiff' , 'VCLog' , 'VCDel' ]
"     " can't use function here ? recursive sourcing ?
"     "let operations[op] = 'vl#dev#versioncontrolsystem#subversion#'.op
"   "endfor
"   "return operations
" "endfunction
" 
" " returns svn file info in a dictionary or 0..
" function! vl#dev#versioncontrolsystem#subversion#SvnInfo(file)
"   let result = vl#lib#system#system#SystemWA(0,'svn',['info',a:file])
"   let d = {}
...
" endfunction
" 
" " returns if this file is located in a directory which can belong to a repo.
" " Thus if it's located within a .svn directory.
" " Doesn't mean a member of (added to) the repository
" function! vl#dev#versioncontrolsystem#subversion#FileInRepoDir(file)
"    return [] != vl#lib#files#filefunctions#WalkUpAndFind(
"          \ vl#lib#files#filefunctions#FileDir(a:file), 
"     \ "glob(vl#lib#files#filefunctions#JoinPaths(path,'.svn'))","all")
" endfunction
" 
" " returns wether the file belongs to the repository (has been added)
" " TODO use status here?
" function! vl#dev#versioncontrolsystem#subversion#FileAdded(file)
"     return  ( filereadable(a:file) && (vl#lib#system#system#System(0,'svn status '.a:file)  !~ '^?\|\n?\|is not a working copy') )
" endfunction
" 
" function! vl#dev#versioncontrolsystem#subversion#HandleAction(action,files,...)
"   call vl#dev#versioncontrolsystem#manager#VCResult(
"         \ '=========== action =================================================')
"   call vl#dev#versioncontrolsystem#manager#VCResult('subversion#HandleAction '.a:action.string(a:files).string(a:000))
" 
"   if a:action == "VCInfo"
"     for f in a:files
"       call vl#dev#versioncontrolsystem#manager#VCResult(
"             \ vl#lib#system#system#SystemWA(0,'svn',['info',f])
"             \ )
"     endfor
"   elseif a:action == "VCStatus"
"       call vl#dev#versioncontrolsystem#manager#VCResult(
"             \ vl#lib#system#system#SystemWA(0,'svn',['status']+a:files)
"             \ )
"   elseif a:action == "VCLog"
"       call vl#dev#versioncontrolsystem#manager#VCResult(
"             \ vl#lib#system#system#SystemWA(0,'svn',['log']+a:files)
"             \ )
"   elseif a:action == "VCDiff"
"       call vl#dev#versioncontrolsystem#manager#VCResult(
"             \ vl#lib#system#system#SystemWA(0,'svn',['diff']+a:files)
"             \ )
"   elseif a:action == "VCCommit" || a:action == "VCAdd"
"     let results = ""
"     " to be able to commit a file we need to add it as well
"     call vl#dev#versioncontrolsystem#manager#VCResult(
"           \ "checking wether files have already been added ..")
"     let filesToAdd = vl#lib#listdict#list#MapIf(a:files, 
"           \ '(!vl#dev#versioncontrolsystem#subversion#FileAdded(Val))', 'Val')
"     if len(filesToAdd) > 0
"       for f in filesToAdd
"         let path = vl#lib#files#filefunctions#WalkUpAndFind(f,
"               \ "glob(vl#lib#files#filefunctions#JoinPaths(path,'.svn'))","all")
"         call reverse(path)
"         call remove(path,0)
"         for p in path
"           let results .= vl#lib#system#system#SystemWA(0,"svn", ["add","-N",p])
"         endfor
"       endfor
"       echo len(filesToAdd).' of '.len(a:files).' have been added'
"     else
"       call vl#dev#versioncontrolsystem#manager#VCResult(
"             \ "yes, all given files are already a member of the repository")
"     endif
"     call vl#dev#versioncontrolsystem#manager#VCResult(results)
"   endif
"   if a:action == 'VCCommit'
"     let listToCommit = []
"     for f in a:files
"       let toCommit = [f]
"       while 1
"         let f = vl#lib#files#filefunctions#FileDir(f)
"         if vl#dev#versioncontrolsystem#subversion#SvnInfo(f)['Schedule'] == 'add'
"           call insert(toCommit, f)
"         else
"           break
"         endif
"       endwhile
"       call extend(listToCommit, toCommit)
"     endfor
"     call vl#dev#versioncontrolsystem#manager#CommentsOnFiles(a:action, listToCommit,
"           \ { 'func' : 'vl#dev#versioncontrolsystem#subversion#HandleAction'
"           \ , 'args' :  ['doCommit', listToCommit] }
"           \ )
"   elseif a:action == 'doCommit'
"     let msgs = a:1
"     let msgAll = vl#lib#listdict#dict#KeyValue(msgs
"           \ , 'comment for all (additional comments will be added below this)'
"           \ , "")
"     echo a:files
"     " single commit? only one comment for all files given?
"     let singleC = 0
"     for f in a:files
"       if msgs[f] != ""
"         let singleC = 1
...
"       for f in a:files
"         let msg = msgAll
"         if vl#lib#listdict#dict#HasKey(msgs,f)
"           let msg .= "\n".msgs[f]
"         endif
"         call vl#dev#versioncontrolsystem#manager#VCResult(
"               \ vl#lib#system#system#SystemWA(0,'svn',['commit','-N',f,'-m',msg])
"               \)
"       endfor
"     else
"       call vl#dev#versioncontrolsystem#manager#VCResult(
"             \ vl#lib#system#system#SystemWA(0,'svn',['commit','-N','-m',msgAll] + a:files)
"             \)
"     endif
"   endif
" endfunction
" end region 'file <dotvim>/autoload/vl/dev/versioncontrolsystem/subversion.vim'
...
" start region 'file <dotvim>/autoload/vl/lib/ui/tagging.vim'
" "|fld   description : Tag buffers and get a list of filenames
" "|fld   keywords : <+ this is used to index / group scripts ?+> 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Sep 03 12:55:41
" "|fld   version: 0.0
" "|fld   dependencies: <+ vim-python, vim-perl, unix-tool sed, ... +>
" "|fld   contributors : <+credits to+>
" "|fld   tested on os : <+credits to+>
...
" function! vl#lib#ui#tagging#TaggingUI()
"   noremap <m-t><m-g> : if exists('b:tagged') <bar> unlet b:tagged <bar>  echo "untagged" <bar> else <bar> let b:tagged=1<cr> <bar> echo "tagged" <bar> endif <cr>
"   " for command line (\ escaped)
"   inoremap <m-t><m-f><m-c> <c-r>=join(
"         \ vl#lib#listdict#list#MapCopy(
"         \ vl#lib#ui#tagging#TaggedFiles(),"substitute(Val,' ','\ ','g')")," ")<cr>
" endfunction
" 
" function! vl#lib#ui#tagging#TaggedFiles(...)
"   exec vl#lib#brief#args#GetOptionalArg('returnAtLeastCurrent',string(0))
"   let files = []
"   " I don't know how to do a expand('%')s:ð
"   for nr in range(1,bufnr('$'))
"     if getbufvar(nr,'tagged') == 1
"       call add(files, bufname(nr))
"     endif
"   endfor
"   " I want relative paths:
"   call map(files, 'vl#lib#files#filefunctions#JoinPaths(getcwd(),v:val)')
"   if files == [] && returnAtLeastCurrent
"     return [expand('%:p')]
"   endif
"   return files
"   using a ugly global var because i really need absolute path values. 
"   " This is the easiest way to get them
"   "let g:taggedfiles=[]
"   "let b = 
"   "bufdo if exists('b:tagged') | call add(g:taggedfiles, expand('%:p") | endif
"   "let tf = g:taggedfiles
"   "unlet g:taggedfiles
"   "return tf
" endfunction
" 
" function! vl#lib#ui#tagging#TaggedFilesEscaped(...)
"   exec vl#lib#brief#args#GetOptionalArg('returnAtLeastCurrent',string(0))
"   let files = vl#lib#ui#tagging#TaggedFiles(returnAtLeastCurrent)
"   call map(files, "substitute(v:val,' ','\\\\ ','g')")
"   return files
" endfunction
" 
" command! UntagAll bufdo if exists('b:tagged') | unlet b:tagged | endif
" end region 'file <dotvim>/autoload/vl/lib/ui/tagging.vim'
...
" start region 'file <dotvim>/autoload/vl/dev/versioncontrolsystem/manager.vim'
" "|fld   description : My own version control system
" "|fld   keywords : svn darcs hg 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Aug 24 01:22:05
" "|fld   version: 0.0
" "|fld   dependencies: svn, darcs, hg, cvs ? ...
" "|fld   maturity: unusable, experimental
" "|fld     os: <+ remove this value if script is os independent +>
" "|
" "|H1__  Documentation
" "|
" "|p     Enhance your productivity using version control systems within vim
" "|H2_   Kind of plugin architecture, See vl#dev#versioncontrolsystem#subversion as example.
" "|
" "|H2__  settings
" "|set   <description of setting(s)>
" "|      "description
" let g:registeredVCSystems=vl#lib#vimscript#scriptsettings#Load('vl.dev.versioncontrolsystem.manager.registeredVCSystems',
"  \ [ 'vl#dev#versioncontrolsystem#subversion' ])
"  "\ , 'vl#dev#versioncontrolsystem#mercurial' 
"  "\ , 'vl#dev#versioncontrolsystem#darcs' 
"  "\ ] )
" 
" 
" "let s:map_leader=vl#lib#vimscript#scriptsettings#Load('vl.dev.versioncontrolsystem.registeredVCSystems', "g")
" 
" " add the version control commands
" function! vl#dev#versioncontrolsystem#manager#VCSUI()
"   " Add is non recursively !
"   let commands = [ 'VCAdd' , 'VCCommit' , 'VCDiff', 'VCInfo'
"    \ , 'VCLog' , 'VCDel' , 'VCMove' , 'VCMemberOfRepo', 'VCStatus' ]
"   let mappings = { 'VCAdd' : "vca"
"         \ , 'VCCommit' : "vcc"
"         \ , 'VCDiff' : "vcf"
"         \ , 'VCInfo' : "vci"
"         \ , 'VCStatus' : "vcs"
...
"         \}
" 
"   " annotate, ... some are missing I think :)
"   for c in commands
"     exec 'command! -complete=file -nargs=* '.c.' call vl#dev#versioncontrolsystem#manager#ActionOnFiles('.string(c).',<f-args>)'
"     exec 'noremap \'.mappings[c].' :exec "'.c.' ".join(vl#lib#ui#tagging#TaggedFilesEscaped(1)," ")<cr>'
"   endfor
" endfunction
" 
" function! vl#dev#versioncontrolsystem#manager#ActionOnFiles(action, ...)
"   call vl#lib#ide#logging#Log('manager#ActionOnFiles action:'.a:action.' opts: '.string(a:000))
"   let files = a:000
"   if len(files) == 0
"     echo "no files specified for action ".a:action.", doing nothing"
"     return
"   endif
" 
"   " check for writing  - ask user to write if modified or file not on disk
"   let filesToAsk = vl#lib#listdict#list#MapIf(files, 
"         \ '!filereadable(Val) && !isdirectory(Val)'
"         \ . '|| bufnr(Val) >= 0 && getbufvar(bufnr(Val), "&modified")'
"         \ ,'Val')
"   let askToWrite = []
"   for f in filesToAsk
"     let b = bufnr(f)
"     let modified = getbufvar(b,'&modified')
"     if ( modified || !filereadable(f) ) && (!isdirectory(f))
"       call add(askToWrite, { 'modified':modified, 'f':f, 'b':b} )
"     endif
"   endfor
"   call vl#lib#listdict#list#MapCopy(askToWrite,
"         \ 'vl#ui#confirm#IfConfirm(1, filereadable(Val["f"]) ? '
"           \ .' "save modified file ".Val["f"]."?" : "save not yet save file ".Val["f"]."?"'
"           \ .' , "sp|buffer ".Val["b"]."|wq" )')
" 
"   " log files which are not on disk
"   let nonExistingFiles = vl#lib#listdict#list#MapIf(files, 
"         \'!filereadable(Val) && ! isdirectory(Val)','Val')
"   if len(nonExistingFiles) > 0
"     call vl#dev#versioncontrolsystem#manager#EchoLog(
"           \ "warning! The following files / directories do not exist on disk!")
"     call vl#lib#listdict#list#ForEach(nonExistingFiles, 
"           \ 'call vl#dev#versioncontrolsystem#manager#EchoLog(Val)')
"   endif
"   " get the possible control systems...
"   let choices = {}
"   for c in g:registeredVCSystems
"     let filesControlledByC = vl#lib#listdict#list#MapIf( 
"           \ files, c.'#FileInRepoDir(Val)', 'Val' )
"     if len(filesControlledByC) > 0
"       let choices[c] = filesControlledByC
"     endif
"   endfor
"   if len(keys(choices)) == 0
"     echoe "sorry, no version control repositories found for files ".string(files)
"   else
"     let cCopy = deepcopy(choices)
"     call map(cCopy, " v:key.' '.len(v:val).' files'")
"     let c = vl#ui#userSelection#LetUserSelectIfThereIsAChoice(
"      \ 'which version control do you want to use?'
"       \ , values(cCopy), "return index" )
"     let selectedVC = keys(cCopy)[c]
"     let files = choices[selectedVC]
"     call call(selectedVC.'#HandleAction', [a:action,files] )
"   endif
" endfunction
" 
" function! vl#dev#versioncontrolsystem#manager#CommentsOnFiles(caption, files, continuation)
"   let ps = vl#lib#listdict#list#MapCopy(a:files,'{'.string('caption').': Val }')
"   call add( ps, { 'caption' : 'comment for all (additional comments will be added below this)'})
"   call vl#ui#userSettings#LetUserEditProperties( ps, a:continuation, a:caption )
" endfunction
" 
" function! vl#dev#versioncontrolsystem#manager#VCResult(string)
"   call vl#ui#resultBuffer#ResultBufferAction('add','VCAction',a:string)
" endfunction
" 
" function! vl#dev#versioncontrolsystem#manager#EchoLog(message)
"   echo a:message
"   call vl#dev#versioncontrolsystem#manager#VCResult(a:message)
" endfunction
" 
" function! Test()
"   call  vl#dev#versioncontrolsystem#manager#ActionOnFiles('Test', ['/tmp/vimsvn/checkout/repo/a'] )
" endfunction
" end region 'file <dotvim>/autoload/vl/dev/versioncontrolsystem/manager.vim'
